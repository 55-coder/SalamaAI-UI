...
// --- In-memory Stores for Appointment and Clinician Management ---
let cliniciansStore = [
  {
    id: "clinician-1",
    name: "Dr. Sara Patel, FACC",
    email: "sara.patel@salama.ai",
    specialty: "Cardiology & Vascular Medicine",
    bio: "Senior Cardiologist specializing in preventive electrophysiology and predictive non-invasive diagnostics. Over 15 years of clinical practice.",
    location: "Sutter Health Medical Center / Virtual Consult",
    availability: ["Monday 09:00 - 12:00", "Wednesday 14:00 - 17:00", "Friday 10:00 - 13:00"],
    avatarUrl: "https://images.unsplash.com/photo-1559839734-2b71ea197ec2?auto=format&fit=crop&q=80&w=200"
  },
  {
    id: "clinician-2",
    name: "Dr. Marcus Vance, FACP",
    email: "marcus.vance@salama.ai",
    specialty: "Internal Medicine & Metabolic Care",
    bio: "Focused on metabolic syndromes, diabetic microvascular damage prevention, and custom clinical health assessments.",
    location: "Stanford Health Clinic / Virtual Consult",
    availability: ["Tuesday 10:00 - 14:00", "Thursday 09:00 - 12:00"],
  ...
  },
  {
    ...
    email: "linda.zhao@salama.ai",
    specialty: "Cerebrovascular Electrophysiology",
    bio: "Renowned expert on stroke risk profiles, long-term blood pressure control pathways, and hypermedication adjustments.",
    location: "UCSF Medical Center / Virtual Consult",
    availability: ["Wednesday 09:00 - 12:00", "Friday 14:00 - 16:00"],
...
let appointmentsStore: any[] = [
  {
    ...
    clinicianName: "Dr. Sara Patel, FACC",
    datetime: "2026-06-15T10:00:00Z",
    type: "virtual",
    status: "pending",
    notes: "Review recent multi-disease predictive scan showing low-risk parameters and check active blood pressure stabilization.",
    createdAt: "2026-06-04T10:00:00Z"
  }
];

let prescriptionsStore: any[] = [
  {
    id: "rx-1",
    patientEmail: "antonynjuguna502@gmail.com",
    patientName: "Antony Njuguna",
    ...
    dosage: "One tablet daily in the morning",
    indications: "Maintain blood pressure target below 120/80 mmHg based on recent telemetry scan readings.",
    status: "active",
    prescribedAt: "2026-05-28T11:00:00Z"
  }
];

// --- Patient Appointments Endpoints ---

// 1. Browse clinicians available for booking
app.get("/api/appointments/appointments/clinicians", (req, res) => {
  res.json(cliniciansStore);
});

// 2. Book an appointment with a clinician
app.post("/api/appointments/appointments/book", (req, res) => {
  const { clinicianId, datetime, type, notes, patientEmail, patientName } = req.body;
  if (!clinicianId || !datetime || !patientEmail) {
    return res.status(400).json({ error: "Missing required booking details." });
  }

  const clinician = cliniciansStore.find(c => c.id === clinicianId) || cliniciansStore[0];
  const newAppt = {
    id: `appt-${Date.now()}`,
    patientEmail: patientEmail.toLowerCase(),
  ...
  // Insert clinician-patient sync notify
  systemLogs.unshift({
    id: `log-${Date.now()}`,
    category: "database",
    level: "info",
    message: `Appointment booked by ${patientName} with ${clinician.name} on ${new Date(datetime).toLocaleDateString()}.`,
    timestamp: new Date().toISOString()
  });
...
app.get("/api/appointments/appointments/me", (req, res) => {
  const { email } = req.query;
  if (!email) {
    return res.status(400).json({ error: "Missing identity query parameter." });
  }
  const filtered = appointmentsStore.filter(a => a.patientEmail.toLowerCase() === String(email).toLowerCase());
  res.json(filtered);
});

// 4. Get a single appointment detail
app.get("/api/appointments/appointments/me/:appointment_id", (req, res) => {
  const { appointment_id } = req.params;
  const appt = appointmentsStore.find(a => a.id === appointment_id);
  if (!appt) {
...
app.delete("/api/appointments/appointments/me/:appointment_id", (req, res) => {
  const { appointment_id } = req.params;
  const index = appointmentsStore.findIndex(a => a.id === appointment_id);
  if (index === -1) {
    return res.status(404).json({ error: "Appointment not found." });
  }
...
// 6. Reschedule a pending appointment
app.patch("/api/appointments/appointments/me/:appointment_id/reschedule", (req, res) => {
  const { appointment_id } = req.params;
  ...
  appointmentsStore[index].datetime = datetime;
  appointmentsStore[index].status = "pending"; // reset status or maintain on reschedule

  systemLogs.unshift({
    id: `log-${Date.now()}`,
    category: "database",
    level: "info",
    message: `Appointment (${appointment_id}) has been rescheduled to ${new Date(datetime).toLocaleDateString()}.`,
    timestamp: new Date().toISOString()
  });
...
// 1. Create or register patient/clinician profile
app.post("/api/clinicians/clinicians/profile", (req, res) => {
  const { name, email, specialty, bio, location, availability } = req.body;
  if (!email || !name) {
    return res.status(400).json({ error: "Missing required fields." });
  }

  const existingIndex = cliniciansStore.findIndex(c => c.email.toLowerCase() === email.toLowerCase());
  const newProfile = {
    id: existingIndex !== -1 ? cliniciansStore[existingIndex].id : `clinician-${Date.now()}`,
    name,
    email: email.toLowerCase(),
    specialty: specialty || "General Cardiology",
    bio: bio || "",
    location: location || "Virtual Consultation Clinic",
    availability: availability || ["Monday-Friday 09:00 - 17:00"],
    avatarUrl: req.body.avatarUrl || "https://images.unsplash.com/photo-1559839734-2b71ea197ec2?auto=format&fit=crop&q=80&w=200"
  };

  if (existingIndex !== -1) {
    cliniciansStore[existingIndex] = newProfile;
  } else {
    cliniciansStore.push(newProfile);
  }

  res.json({ success: true, clinician: newProfile });
});

// 2. Get my clinician profile
app.get("/api/clinicians/clinicians/me", (req, res) => {
  const { email } = req.query;
  if (!email) {
    return res.status(400).json({ error: "Missing email parameter." });
  }
  const clinician = cliniciansStore.find(c => c.email.toLowerCase() === String(email).toLowerCase());
  if (!clinician) {
    // Create lazy clinical demo profile to fail gracefully
    const defaultClinician = {
      id: "clinician-1",
      name: "Dr. Sara Patel, FACC",
      email: String(email).toLowerCase(),
      specialty: "Cardiology & Vascular Medicine",
      bio: "Senior Cardiologist specializing in preventive electrophysiology and predictive non-invasive diagnostics.",
      location: "Sutter Health Medical Center / Virtual Consult",
      availability: ["Monday 09:00 - 12:00", "Wednesday 14:00 - 17:00", "Friday 10:00 - 13:00"],
      avatarUrl: "https://images.unsplash.com/photo-1559839734-2b71ea197ec2?auto=format&fit=crop&q=80&w=200"
    };
    cliniciansStore.push(defaultClinician);
    return res.json(defaultClinician);
  }
  res.json(clinician);
});

// 3. Update active clinician profile
app.patch("/api/clinicians/clinicians/me", (req, res) => {
  const { email, name, specialty, bio, location, availability } = req.body;
  if (!email) {
    return res.status(400).json({ error: "Missing clinician email context." });
  }
  const idx = cliniciansStore.findIndex(c => c.email.toLowerCase() === email.toLowerCase());
  if (idx === -1) {
    return res.status(404).json({ error: "Clinician profile not found." });
  }

  if (name) cliniciansStore[idx].name = name;
  if (specialty) cliniciansStore[idx].specialty = specialty;
  if (bio) cliniciansStore[idx].bio = bio;
  if (location) cliniciansStore[idx].location = location;
  if (availability) cliniciansStore[idx].availability = availability;

  res.json({ success: true, clinician: cliniciansStore[idx] });
});

// 4. List linked patients — optionally filter by risk level
app.get("/api/clinicians/clinicians/me/patients", (req, res) => {
  const { risk_level } = req.query;

  // Derive active patient entries from diagnostic assessments
  const uniquePatientsMap: { [email: string]: any } = {};
  assessments.forEach(ass => {
    const emailKey = ass.patientEmail.toLowerCase();
    if (!uniquePatientsMap[emailKey] || new Date(ass.timestamp) > new Date(uniquePatientsMap[emailKey].timestamp)) {
      uniquePatientsMap[emailKey] = ass;
    }
  });

  let patientsList = Object.values(uniquePatientsMap).map(ass => ({
    patientId: ass.patientEmail,
    name: ass.patientName,
    email: ass.patientEmail,
    latestAssessment: ass
  }));

  if (risk_level) {
    patientsList = patientsList.filter(p => p.latestAssessment.riskCategory.toLowerCase() === String(risk_level).toLowerCase());
  }

  res.json(patientsList);
});

// 5. Search linked patients by name or email
app.get("/api/clinicians/clinicians/me/patients/search", (req, res) => {
  const { q } = req.query;
  ...
  if (querystring) {
    patientsList = patientsList.filter(p => p.name.toLowerCase().includes(querystring) || p.email.toLowerCase().includes(querystring));
  }

  res.json(patientsList);
});

// 6. Get full profile, latest assessment, and risk data for a patient
app.get("/api/clinicians/clinicians/me/patients/:patient_id", (req, res) => {
  const { patient_id } = req.params;
  const patientEmail = decodeURIComponent(patient_id).toLowerCase();

  const patientAssessments = assessments.filter(a => a.patientEmail.toLowerCase() === patientEmail);
  if (patientAssessments.length === 0) {
    return res.status(404).json({ error: "Patient records not found." });
  }

  // Sorted latest first
  patientAssessments.sort((a,b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());

  res.json({
    email: patientEmail,
    name: patientAssessments[0].patientName,
    latestAssessment: patientAssessments[0],
    history: patientAssessments
  });
});

// 7. Schedule action or scheduler creation by clinician
app.post("/api/clinicians/clinicians/me/appointments", (req, res) => {
  const { patientEmail, patientName, datetime, type, notes, clinicianEmail } = req.body;
  if (!patientEmail || !datetime || !clinicianEmail) {
    return res.status(400).json({ error: "Missing appointment details." });
  }

  const clinician = cliniciansStore.find(c => c.email.toLowerCase() === clinicianEmail.toLowerCase()) || cliniciansStore[0];
  const newAppt = {
    id: `appt-${Date.now()}`,
    patientEmail: patientEmail.toLowerCase(),
    ...
    datetime,
    type: type || "virtual",
    status: "confirmed", // clinicians directly scheduling marks confirmed
    notes: notes || "",
    createdAt: new Date().toISOString()
  };
  ...
  systemLogs.unshift({
    ...
    category: "database",
    level: "info",
    message: `${clinician.name} scheduled directly a ${type} appointment with ${patientName}.`,
    timestamp: new Date().toISOString()
  });
...
// 8. List clinician appointments
app.get("/api/clinicians/clinicians/me/appointments", (req, res) => {
  const { email } = req.query;
  if (!email) {
    return res.status(400).json({ error: "Missing clinician email identity." });
  }
  const clinician = cliniciansStore.find(c => c.email.toLowerCase() === String(email).toLowerCase());
  if (!clinician) {
    return res.json([]);
  ...
  const matches = appointmentsStore.filter(a => a.clinicianId === clinician.id);
  res.json(matches);
});

// 9. Clinician edits appointment details, status, or notes
app.patch("/api/clinicians/clinicians/me/appointments/:appointment_id", (req, res) => {
  const { appointment_id } = req.params;
  const { status, datetime, notes } = req.body;

  const idx = appointmentsStore.findIndex(a => a.id === appointment_id);
  if (idx === -1) {
    return res.status(404).json({ error: "Appointment not found." });
  }

  if (status) appointmentsStore[idx].status = status;
  if (datetime) appointmentsStore[idx].datetime = datetime;
  if (notes) appointmentsStore[idx].notes = notes;

  systemLogs.unshift({
    id: `log-${Date.now()}`,
    category: "database",
    level: "info",
    message: `Clinician modified appointment (${appointment_id}): Status=${status || appointmentsStore[idx].status}`,
    timestamp: new Date().toISOString()
  });

  res.json({ success: true, appointment: appointmentsStore[idx] });
});

// 10. Assign prescriptions
app.post("/api/clinicians/clinicians/me/prescriptions", (req, res) => {
  const { patientEmail, patientName, clinicianEmail, medicationName, dosage, indications } = req.body;
  if (!patientEmail || !medicationName || !clinicianEmail) {
    return res.status(400).json({ error: "Missing vital prescription parameters." });
  }
  ...
  systemLogs.unshift({
    ...
    category: "database",
    level: "info",
    message: `${clinician.name} prescribed ${medicationName} to ${patientName}.`,
    timestamp: new Date().toISOString()
  });
...
// 11. Retrieve prescriptions
app.get("/api/clinicians/clinicians/me/prescriptions", (req, res) => {
  const { patient_email, clinician_email } = req.query;
  let filtered = prescriptionsStore;

  if (patient_email) {
    filtered = filtered.filter(p => p.patientEmail.toLowerCase() === String(patient_email).toLowerCase());
  }
  if (clinician_email) {
    const clinician = cliniciansStore.find(c => c.email.toLowerCase() === String(clinician_email).toLowerCase());
    if (clinician) {
      filtered = filtered.filter(p => p.clinicianId === clinician.id);
    } else {
      return res.json([]);
    }
  }
